...
class DB_Tarefa {
    ...
    async concluir(id, user_id) {
        await query("UPADATE Tarefa SET status = true WHERE status = false AND id = $1 AND user_id = $2", [id, user_id])
    } 

    async deletar(id, user_id) {
        await query("DELETE FROM Tarefa WHERE status = false AND id = $1 AND user_id = $2", [id, user_id])
    }
}
